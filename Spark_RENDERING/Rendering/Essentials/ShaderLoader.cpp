...
{
    GLuint ShaderLoader::CreateProgram(const std::string& vertexSource, const std::string& fragmentSource)
    {
        ...
        if (!LinkShaderProgram(program, vertex, fragment))
        {
    ...
    bool ShaderLoader::IsProgramValid(GLuint program)
    {
        ...
        if (status != GL_TRUE)
        {
            ...
            glGetShaderInfoLog(program, maxLength, &maxLength, errorLog.data());
    ...
    bool ShaderLoader::LinkShaderProgram(GLuint program, GLuint vertexShader, GLuint fragmentShader)
    {
