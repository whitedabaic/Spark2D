...
namespace SPARK_CORE {
	...
	FollowCamera::FollowCamera(SPARK_RENDERING::Camera2D& camera, const ECS::Entity& entity, const FollowCamParams& params)
		: m_Camera{ camera }, m_Params{ params }, m_Entity{ entity }
	{
		SPARK_ASSERT(m_Params.scale > 0.f && "The scale must be greater than zero!");
