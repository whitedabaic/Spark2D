...
namespace SPARK_CORE::Systems {
	RenderUISystem::RenderUISystem(SPARK_CORE::ECS::Registry& registry)
		: m_Registry{ registry }
		, m_pSpriteRenderer{ std::make_unique<SPARK_RENDERING::SpriteBatchRenderer>() }
		, m_pTextRenderer{ std::make_unique<SPARK_RENDERING::TextBatchRenderer>() }
		, m_pCamera2D{ nullptr }
	{
		m_pCamera2D = std::make_unique<SPARK_RENDERING::Camera2D>(640, 480);
		m_pCamera2D->Update();
	}
	...
	void RenderUISystem::Update(entt::registry& registry)
	{
		...
		if (!pFontShader)
		{
			SPARK_ERROR("Font shader not found in asset manager!");
			return;
		}
		...
		for (auto entity : textView)
		{
			...
			if (!pFont)
			{
				auto font = text.sFontName;
				SPARK_ERROR("Font [{0}] was not found in asset manager!", font);
				continue;
			}
				...
				transform.scale.y > 1.f || transform.scale.y < 1.f)
			{
				...
				model = glm::translate(model, glm::vec3{ (pFont->GetFontSize() * transform.scale.x) * 0.5f,
					(pFont->GetFontSize() * transform.scale.y) * 0.5f, 0.f });
