...
namespace SPARK_CORE::Systems {
	RenderShapeSystem::RenderShapeSystem(SPARK_CORE::ECS::Registry& registry)
		: m_Registry{ registry }
	...
	void RenderShapeSystem::Update()
	{
		...
		for (auto entity : boxView)
		{
				...
				transform.scale.y > 1.f || transform.scale.y < 1.f)
			{
				...
				model = glm::translate(model, glm::vec3{ (boxCollider.width * transform.scale.x) * 0.5f,
					(boxCollider.height * transform.scale.y) * 0.5f, 0.f });
