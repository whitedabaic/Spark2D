...
void SPARK_CORE::Scripting::RendererBinder::CreateRenderingBind(sol::state& lua, SPARK_CORE::ECS::Registry& registry)
{
	...
	if (!renderer)
	{
		SPARK_ERROR("Failed to bind the Renderer to Lua! Not in the Registry context!");
		return;
	}
	...
	if (!camera)
	{
		SPARK_ERROR("Failed to bind the camera to lua - Failed to get the camera from the registry context!");
		return;
	}
