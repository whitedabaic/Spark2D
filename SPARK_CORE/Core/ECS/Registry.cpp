...
using namespace SPARK_CORE::Utils;
...
void SPARK_CORE::ECS::Registry::CreateLuaRegistryBind(sol::state& lua, Registry& registry)
{
	...
	lua.new_usertype<entt::runtime_view>(
		...
		[&](entt::runtime_view& view, const sol::variadic_args& va)
		{
			for (const auto& type : va)
			{
				...
				const auto exclude_view = InvokeMetaFunction(
					GetIdType(type),
					"exclude_component_from_view"_hs,
					&registry, view
				);

				view = exclude_view ? exclude_view.cast<entt::runtime_view>() : view;
			}
		}
	);
