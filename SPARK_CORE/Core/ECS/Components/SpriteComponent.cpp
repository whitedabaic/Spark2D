...
void SPARK_CORE::ECS::SpriteComponent::CreateSpriteLuaBind(sol::state& lua, SPARK_CORE::ECS::Registry& registry)
{
	...
	lua.new_usertype<SpriteComponent>(
		...
		sol::factories(
			[](const std::string& textureName, float width, float height, int start_x, int start_y, int layer) {
				return SpriteComponent{
					...
					.height = height,
					.uvs = UVs{},
					.color = SPARK_RENDERING::Color{255, 255, 255, 255},
					.start_x = start_x,
					.start_y = start_y,
					.layer = layer,
		...
		"hidden", &SpriteComponent::bHidden,
		...
		"generate_uvs", [&](SpriteComponent& sprite) {
			...
			if (!texture)
			{
				SPARK_ERROR("Failed to generate uvs -- Texture [{}] -- Does not exists or invalid", sprite.texture_name);
				return;
			}
