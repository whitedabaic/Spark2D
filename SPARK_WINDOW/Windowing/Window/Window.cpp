...
namespace SPARK_WINDOWING
{
	...
	Window::Window(const std::string& title, int width, int height, int x_pos, int y_pos, bool v_sync, Uint32 flags)
		: m_pWindow(nullptr), m_GLContext(), m_sTitle(title), m_Width(width),
		m_Height(height), m_XPos(x_pos), m_YPos(y_pos), m_WindowFlags(flags)
	{
		...
		if (v_sync)
		{
			...
			std::cout << "Window Created Successfully with VSYNC enabled.\n";
		}
	}
