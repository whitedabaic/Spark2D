...
namespace SPARK_EDITOR {
    ...
    bool Application::Initialize()
    {
		...
		SPARK_PHYSICS::PhysicsWorld pPhysicsWorld = std::make_shared<b2World>(b2Vec2{ 0.f, 9.8f });

		if (!m_pRegistry->AddToContext<SPARK_PHYSICS::PhysicsWorld>(pPhysicsWorld))
		{
			SPARK_ERROR("Failed to add the Physics world to the registry context!");
			return false;
		}

		auto pPhysicsSystem = std::make_shared<SPARK_CORE::Systems::PhysicsSystem>(*m_pRegistry);

		if (!m_pRegistry->AddToContext<std::shared_ptr<SPARK_CORE::Systems::PhysicsSystem>>(pPhysicsSystem))
		{
			SPARK_ERROR("Failed to add the Physics system to the registry context!");
			return false;
		}
	...
	void Application::Update()
	{
		...
		auto& pPhysicsWorld = m_pRegistry->GetContext<SPARK_PHYSICS::PhysicsWorld>();
		pPhysicsWorld->Step(
			1.f / 60.f,
			8,
			3
		...
		auto& pPhysicsSystem = m_pRegistry->GetContext<std::shared_ptr<SPARK_CORE::Systems::PhysicsSystem>>();
