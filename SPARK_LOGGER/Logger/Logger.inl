...
namespace SPARK_LOGGER
{
	...
	void Logger::Log(const std::string& message, Args&&... args)
	{
		...
		std::stringstream ss;
		ss << "SPARK [INFO]: " << CurrentDateTime() << " - " << std::vformat(message, std::make_format_args(std::forward<Args>(args)...));
	...
	void Logger::Warn(const std::string& message, Args&&... args)
	{
		...
		std::stringstream ss;
		ss << "SPARK [WARN]: " << CurrentDateTime() << " - " << std::vformat(message, std::make_format_args(std::forward<Args>(args)...));
